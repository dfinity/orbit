...
const parseReleaseMode = (releaseMode?: string): 'alpha' | 'beta' | 'rc' | 'prod' => {
  if (!releaseMode) {
    return 'prod';
  }
...
program
  ...
  .option('-P, --pre-release', 'Specify the type of pre-release version to use (alpha, beta or rc)')
  .action(async options => {
    const projects = options.projects
      ? options.projects.split(',').map((group: string) => group.trim())
      : undefined;

    const releaseMode = parseReleaseMode(options.preRelease);
    ...
    const { workspaceVersion, projectsVersionData } = await releaseVersion({
      ...
      preid: releaseMode !== 'prod' ? releaseMode : undefined,
    });
