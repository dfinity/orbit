...
ENV RUSTUP_HOME=/opt/rustup \
    ...
    FNM_DIR=/opt/fnm \
    MALLOC_CONF="narenas:1,abort:false"
...
RUN BUILD_MODE=production npx nx run station:create-artifacts
...
RUN BUILD_MODE=production npx nx run upgrader:create-artifacts
...
RUN BUILD_MODE=production npx nx run control-panel:create-artifacts
