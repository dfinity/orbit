  ...
  for (const policy of policies) {
    walkQuorumRules(policy.rule, namedByUUID, (kind, approvers, minApproved, path) => {
      ...
      if (resolved.length === 0 && minApproved > 0) {
        // `min_approved` is a vote count for Quorum and a percentage (0-100)
        // for QuorumPercentage. The empty-set bug fires identically in both
        // cases (`percentage * 0 / 100 == 0`), but the report wording differs
        // so operators read it correctly.
        const requirement =
          kind === 'QuorumPercentage'
            ? `${minApproved}% of approvers`
            : `${minApproved} approval(s)`;
        findings.push({
          checkId: 'quorum.empty-approver-set',
          severity: 'blocker',
          location: `policy ${policy.id} (${describeRequestSpecifier(policy.specifier)}) — ${path}`,
          message: `${kind} rule asks for ${requirement} but ${describeSpecifier(approvers)} currently resolves to 0 active users. Next matching request will auto-approve.`,
