...
use station_api::{
    NotificationTypeDTO, RequestAutoApprovedNotificationDTO, RequestCreatedNotificationDTO,
    RequestFailedNotificationDTO, RequestRejectedNotificationDTO,
};
use uuid::Uuid;
    ...
    fn try_from(model: NotificationType) -> Result<NotificationTypeDTO, NotificationMapperError> {
        Ok(match model {
            ...
            NotificationType::RequestCreated(ctx) => {
                ...
                let (account_id, user_id) = ids_from_request(&request);
            ...
            NotificationType::RequestAutoApproved(ctx) => {
                let request = REQUEST_REPOSITORY
                    .get(&Request::key(ctx.request_id))
                    .ok_or(NotificationMapperError::RequestNotFound {
                        request_id: ctx.request_id,
                ...
                let (account_id, user_id) = ids_from_request(&request);
                NotificationTypeDTO::RequestAutoApproved(RequestAutoApprovedNotificationDTO {
                    request_id: Uuid::from_bytes(ctx.request_id).to_string(),
                    operation_type: RequestOperationType::from(request.operation).into(),
                    account_id: account_id.map(|id| Uuid::from_bytes(id).to_string()),
                    user_id: user_id.map(|id| Uuid::from_bytes(id).to_string()),
...
// Returns account and user ids from a request.
fn ids_from_request(request: &Request) -> (Option<[u8; 16]>, Option<[u8; 16]>) {
