...
export const viteConfig = (
  ...
  defineConfig(_ => {
    ...
    const mode = opts.mode ? opts.mode : MODE;
    const base = opts.base ? opts.base : '/';
    const optimized = opts.optimized ? opts.optimized : OPTIMIZED_BUILD;
    const outDir = opts.outDir ? opts.outDir : resolve(__dirname, '../../dist');
    ...
    return {
      ...
      appType: 'mpa',
      server: {
        port: opts.dev?.port ? opts.dev.port : 5173,
      },
      preview: {
        port: opts.preview?.port ? opts.preview.port : 4173,
      },
          ...
          output: [
            {
              ...
              manualChunks: id => {
                ...
                if (
                  isNodeModule &&
                  ['/vue-router/', '/pinia/', '/vuetify/', '/vue-i18n/', '/@intlify/'].some(
                    vendor => folder.includes(vendor),
                  )
                ) {
                  return 'vendor';
                }
